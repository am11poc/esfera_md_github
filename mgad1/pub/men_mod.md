# Cua d'elaboració (models)

* [Què és](men_mod.md#que-es)
* [Com s’hi accedeix](men_mod.md#com-shi-accedeix)
* [Quines operacions s'hi poden fer](men_mod.md#quines-operacions-shi-poden-fer)

## Què són

En aquesta opció del menú **Publicacions** es troben els models que l'usuari ha elaborat.
  
![](../../_media/esfera/mgac/btn-pujar.gif)![](../../_media/esfera/mgac/pujar0.png)

---

## Com s’hi accedeix

Per accedir-hi, heu de seleccionar l'opció del menú **Cua d'elaboració (models)** del mòdul **Publicacions**.

![](../../_media/esfera/mgac/pub/cua_mod_01.png)*Imatge 1 - Pantalla per seleccionar Cua d'elaboració (models)*
  
![](../../_media/esfera/mgac/btn-pujar.gif)![](../../_media/esfera/mgac/pujar0.png)

---

## Quines operacions s'hi poden fer

La pantalla que es mostra conté informació dels models que s'han elaborat.
  
  
L'**estat** dels models indica si s'estan elaborant (**En execució**), si s'han elaborat (**Finalitzada**) o si s'ha produït un error (**Error**).  
  
![](../../_media/esfera/mgac/pub/cua_mod_02.png)*Imatge 2 - Cua d'elaboració de models* 
  
Es poden fer dues operacions:

* [Visualitzar els models](men_mod.md#visualitzar-els-models)
* [Esborrar els models](men_mod.md#esborrar-els-models)

### Visualitzar els models

Per visualitzar un model, cal fer clic a la icona ![](../../_media/esfera/mgac/editar.png).

### Esborrar els models

Per esborrar un model, cal fer clic a la casella de selecció i prémer el botó ![](../../_media/esfera/mgac/b_esborra.png).

![](../../_media/esfera/mgac/btn-pujar.gif)![](../../_media/esfera/mgac/pujar0.png)

---